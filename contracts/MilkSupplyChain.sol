...
contract MilkSupplyChain {
    ...
    // Hàm cập nhật thông tin sản phẩm sữa
    function updateMilkProduct(
        ...
        require(
            keccak256(bytes(participants[msg.sender].role)) ==
                keccak256(bytes("MANUFACTURER")),
            "Chi cho phep MANUFACTURER"
        );
        require(
            bytes(milkBoxes[_serialNumber].serialNumber).length != 0,
            "MilkBox khong ton tai"
        );

        // Chỉ cho phép sửa khi hộp sữa ở trạng thái Created
        require(
            milkBoxes[_serialNumber].status == MilkBoxStatus.Created,
            "Chi duoc sua khi o trang thai Created"
        );

        // Cập nhật thông tin
        milkBoxes[_serialNumber].boxName = _boxName;
        milkBoxes[_serialNumber].manufacturingDate = _manufacturingDate;
        milkBoxes[_serialNumber].expirationDate = _expirationDate;
    }

    // Hàm cập nhật thông tin lô sữa
    function updateMilkBatch(
        uint256 _batchId,
        ...
        require(
            keccak256(bytes(participants[msg.sender].role)) ==
                keccak256(bytes("FARM")),
            "Chi cho phep FARM"
        );
        require(milkBatches[_batchId].batchId != 0, "Batch khong ton tai");

        // Chỉ cho phép sửa khi lô sữa ở trạng thái Created
        require(
        ...
        // Cập nhật thông tin
        milkBatches[_batchId].batchName = _batchName;
        milkBatches[_batchId].milkingDate = _milkingDate;
        milkBatches[_batchId].volume = _volume;
        milkBatches[_batchId].farmOwner = _farmOwner;
        milkBatches[_batchId].approvedBy = _approvedBy;
    }
    ...
    function approveMilking(
        ...
        require(
            keccak256(bytes(participants[msg.sender].role)) ==
                keccak256(bytes("NUTRITION_AUTHORITY")),
    ...
    function approveProcessingBox(string memory _serialNumber) public {
        ...
        require(
            keccak256(bytes(participants[msg.sender].role)) ==
                keccak256(bytes("FOOD_SAFETY_AUTHORITY")),
    ...
    function approveTransportBox(string memory _serialNumber) public {
        ...
        require(
            keccak256(bytes(participants[msg.sender].role)) ==
                keccak256(bytes("TRANSPORT_AUTHORITY")),
    ...
    function updateBoxStatusAtRetail(
        string memory _serialNumber,
        MilkBoxStatus _newStatus
    ) public {
        require(participants[msg.sender].isActive, "Participant khong active");
        require(
            keccak256(bytes(participants[msg.sender].role)) ==
    ...
    function getMilkBoxDetails(
        string memory _serialNumber
    ) public view returns (MilkBoxDetail memory) {

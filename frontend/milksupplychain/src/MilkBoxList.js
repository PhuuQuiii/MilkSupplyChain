...
const MilkBoxList = () => {
  ...
  const { currentAccount, accountList, milkBoxStatus } =
  ...
  const handleApprove = (serial, boxName) => {
    navigate(
      `/master/san-pham-sua-duyet?serialNumber=${serial}&boxName=${boxName}`
    );
  };
    ...
    <div className="container fade-in">
      ...
      <div className="form-group">
        ...
        <div className="flex gap-4 mb-4">
          <button onClick={fetchMilkBoxes} className="btn btn-primary mt-4">
            Tìm kiếm hộp sữa
          </button>
          {currentAccount && currentAccount.role === "MANUFACTURER" && (
            <button
                  ...
                  {currentAccount &&
                    currentAccount.role === "FOOD_SAFETY_AUTHORITY" && (
                      <td style={{ padding: "3px" }} className="actions">
                        <button
                          onClick={() =>
                            handleApprove(box.serialNumber, box.boxName)
                          }
